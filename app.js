...
function render(d) {
  ...
  const truckTotal = truckData.reduce((acc, curr) => ({
    august: acc.august + (curr.august || 0),
    september: acc.september + (curr.september || 0),
    daily: acc.daily + (curr.daily || 0)
  ...
  const prodTotal = prodData.reduce((sum, curr) => sum + (curr.qty || 0), 0);
  ...
  const stockTotal = stockData.reduce((sum, curr) => sum + (curr.volume || 0), 0);
  ...
  $('monthly').innerHTML = m.map(x => `
    <div>${x.month}</div><div>${num(x.y2025_1)}</div><div>${num(x.y2026_1)}</div>
    <div>${x.month2}</div><div>${num(x.y2025_2)}</div><div>${num(x.y2026_2)}</div>
  `).join('') + `<div class="mh">Total</div><div style="grid-column:span 2;font-weight:bold;background:#1a1a1a">${num(mt.y2025)}</div><div style="grid-column:span 3;font-weight:bold;background:#1a1a1a">${num(mt.y2026)}</div>`;
  ...
  $('alphaRows').innerHTML = (d.alpha || []).map(x => `
    <div class="alpha-row"><b>${x.berth}</b><span>${x.vessel}</span><span>${x.materials || '-'}</span><span>${x.discharge || '0%'}</span><span>${x.balance || '0%'}</span><span>${x.progress || '0%'}</span><span>${x.time || '0:00'}</span><span>${x.remarks || '-'}</span><span>${x.equip || '0'}</span><span class="activity">${formatActivityTime(x.activity_time)}</span></div>
  `).join('');

  const f = d.foreign || {};
  $('foreignRow').innerHTML = `
    <span><b>${f.berth}</b></span><span>${f.vessel}</span><span>${f.materials || '-'}</span><span>${f.discharge || '-'}</span><span>${f.balance || '-'}</span><span>${f.progress || '-'}</span><span>${f.time || '-'}</span><span>${f.remarks || '-'}</span><span>${f.equip || '0'}</span><span class="activity">${formatActivityTime(f.activity_time)}</span>
  `;
